...
ESX.RegisterCommand('setjob3', 'admin', function(xPlayer, args, showError)
	if not ESX.DoesJobExist(args.job3, args.grade3) then
		return showError(TranslateCap('command_setjob_invalid'))
	end
	...
	if Config.AdminLogging then
		ESX.DiscordLogFields("UserActions", "Set Job3 /setjob3 Triggered!", "pink", {
			...
			{ name = "ID",     value = xPlayer.source,     inline = true },
			{ name = "Target", value = args.playerId.name, inline = true },
			{ name = "Job3",    value = args.playerId,      inline = true },
			{ name = "Grade3",  value = args.playerId,      inline = true },
		})
	end
end, true, {
	help = TranslateCap('command_setjob'),
...
ESX.RegisterCommand('job3', { "user", "admin" }, function(xPlayer, _, _)
	print(('%s, your job3 is: ^5%s^0 - ^5%s^0'):format(xPlayer.getName(), xPlayer.getJob3().name,
		xPlayer.getJob3().grade_label))
end, true)
...
ESX.RegisterCommand('info2', { "user", "admin" }, function(xPlayer)
	...
	print(('^2ID: ^5%s^0 | ^2Name: ^5%s^0 | ^2Group: ^5%s^0 | ^2Job2: ^5%s^0'):format(xPlayer.source, xPlayer.getName(),
...
ESX.RegisterCommand('info3', { "user", "admin" }, function(xPlayer)
	...
	print(('^2ID: ^5%s^0 | ^2Name: ^5%s^0 | ^2Group: ^5%s^0 | ^2Job3: ^5%s^0'):format(xPlayer.source, xPlayer.getName(),
		xPlayer.getGroup(), job3))
end, true)
